...
theorem completeBipartiteGraph_homomorphism (V W : Type*) :

  ∀ (v w : Sum V W), ∃ (f : Sum V W → Bool) ,
    (completeBipartiteGraph V W).Adj v w ↔ (f v ≠ f w) := by
  intros v w
...
theorem completeBipartiteGraph_homomorphism2 (V W : Type*) :
  ∀ (v w : Sum V W), ∃ (f : Sum V W → Bool) ,
    (completeBipartiteGraph V W).Adj v w ↔ (f v ≠ f w) := by
  intro v w
  use Sum.elim (fun _ ↦ true) (fun _ ↦ false)
